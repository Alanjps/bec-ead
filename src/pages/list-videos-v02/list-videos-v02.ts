...
@IonicPage({
  name: 'list-videos-v02-page'
})
@Component({
  selector: 'page-list-videos-v02',
  templateUrl: 'list-videos-v02.html',
})
export class ListVideosV02Page {
  ...
  expItems: any = [];
  public titleTesteira: string = 'Mi agenda';
      ...
      if (value != null && value.length > 0){
        ...
        this.storage.get('clienteCompanyId').then((companyId) => {
          this.storage.get('clienteId').then((clienteId) => {
            ...
            loading.present();
            this.http.getAll('/api/conteudos', { company_id: companyId, user_id: clienteId, type: "DEFAULT" })
              .subscribe((data:any) => {
                loading.dismiss();
