      ...
      if (value != null && value.length > 0){
        ...
        this.storage.get('clienteCompanyId').then((companyId) => {
          this.storage.get('clienteId').then((clienteId) => {
            ...
            this.http.getAll('/api/projects-ids', { company_id: companyId, user_id: clienteId, type: "ICON5" })
            .subscribe(async (ids:any) => {
              ids.map( async(id) => {           
                ...
                await new Promise((resolve, reject)=>{
                  this.http.getAll('/api/conteudos', { company_id: companyId, user_id: clienteId, type: "ICON5", project_id: id})
                  .subscribe((data:any) => {
